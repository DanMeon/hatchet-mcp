...
Per-version roadmap + the **SSOT for the active spec index**. Every spec's Status / GA / Target is tracked on this page. See [docs/CONVENTIONS.md](../CONVENTIONS.md) for the documentation policy.
...
## Implementation / verification logs (Frozen)

Logs written after work completes. Frozen — no changes.
...
- **Spec lifecycle**: Draft → (GA) → Frozen → (if needed) → Superseded by …. Details: [CONVENTIONS.md § Adding a New Spec](../CONVENTIONS.md)
